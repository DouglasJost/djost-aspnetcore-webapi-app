...
namespace OpenAiChatCompletions.Repositories
{
  public class OpenAiChatCompletionRepository : IOpenAiChatCompletionRepository
  {
    ...
    public OpenAiChatCompletionRepository(KeyVaultService keyVaultService)
    {
      ...
      ChatCompletionServiceProviderType chatCompletionServiceProvider)
    {
      ...
      string openAiApiToken = await _keyVaultService.GetSecretValueAsync(KeyVaultSecretNames.ChatCompletions_OpenAI_Token);
      string openAiApiUrl = await _keyVaultService.GetSecretValueAsync(KeyVaultSecretNames.ChatCompletions_OpenAI_Url);
      ...
      string azureApiKey = await _keyVaultService.GetSecretValueAsync(KeyVaultSecretNames.ChatCompletions_Azure_OpenAI_Key);
      string azureApiUrl = await _keyVaultService.GetSecretValueAsync(KeyVaultSecretNames.ChatCompletions_Azure_OpenAI_Url);
      if (chatCompletionServiceProvider == ChatCompletionServiceProviderType.AzureOpenAI)
      {
        if (string.IsNullOrWhiteSpace(azureApiKey) || string.IsNullOrWhiteSpace(azureApiUrl))
        {
      ...
      var url = (chatCompletionServiceProvider == ChatCompletionServiceProviderType.OpenAI) ? openAiApiUrl : azureApiUrl;
